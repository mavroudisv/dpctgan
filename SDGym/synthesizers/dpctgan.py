...
class Cond(object):
    def __init__(self, data, output_info):
        ...
        # return
        self.model = []
        self.uniform_mask_enabled = UNIFORM_MASK_ENABLED
        ...
        for item in output_info:
            if item[1] == 'tanh':
                ...
                if not self.uniform_mask_enabled:
                    tmp = np.sum(data[:, st:ed], axis=0)
                    tmp = np.log(tmp + 1)
                    tmp = tmp / np.sum(tmp)
                else:
                    tmp = np.array([1/item[0]] * item[0])
...
class dpCTGANSynthesizer(BaseSynthesizer):
    """docstring for dpCTGANSynthesizer."""
    ...
    def fit(self, train_data, categorical_columns=tuple(), ordinal_columns=tuple()):
        ...
        self.generator = convert_batchnorm_modules(self.generator).to(self.device)
        self.generator.apply(weights_init)
        engine = self._init_dp_engine(self.generator, self.batch_size, len(train_data))
        engine.attach(optimizerG)
        ...
        for i in range(self.epochs):
            for id_ in range(steps_per_epoch):
                ...
                #epsilon, best_alpha = optimizerG.privacy_engine.get_privacy_spent(self.target_delta)
                #print("(ε = %.2f, δ = %.2f) for α = %.2f" % (epsilon, self.target_delta, best_alpha))
